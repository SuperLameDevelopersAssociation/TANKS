...
public class ArcReactor_Arc : MonoBehaviour
{
    ...
    protected void SetFlares(int n)
    {
        ...
        if (arcs[n].flaresOptions.startFlare.enabled)
        {
            ...
            startFlare.brightness = GetFlareBrightness(currentCamera.transform.position, resultingShape[0], arcs[n].flaresOptions.startFlare,
        ...
        if (arcs[n].flaresOptions.endFlare.enabled)
        {
            ...
            if (arcs[n].sizeOptions.onlyStartWidth)
                endFlare.brightness = GetFlareBrightness(currentCamera.transform.position, resultingShape[resultingShape.Length - 1], arcs[n].flaresOptions.endFlare,
                                                         arcs[n].sizeOptions.startWidthCurve.Evaluate(elapsedTime / lifetime) / maxStartWidth[n]) * multiplier;
            else
                endFlare.brightness = GetFlareBrightness(currentCamera.transform.position, resultingShape[resultingShape.Length - 1], arcs[n].flaresOptions.endFlare,
    ...
    void Start()
    {
        ...
        for (int i = 0; i < arcs.Length; i++)
        {
			...
			if (arcs[i].sizeOptions.segmentLength <= 0)
				Debug.LogWarning(gameObject.name + " : Segment Length of Arc #"+i.ToString()+" is set to zero or lower. It would cause unexpected behaviour or division by zero errors.");
        ...
        for (int n = 0; n < arcs.Length; n++)
        {
			...
			for (int i = 0; i < arcs[n].emissionOptions.Length; i++)
				particleBuffers[n][i] = new ParticleSystem.Particle[arcs[n].emissionOptions[i].shurikenPrefab.maxParticles];
        }
        ...
        for (int n = 0; n < arcs.Length; n++)
        {
            ...
            for (int i = 0; i < arcs[n].emissionOptions.Length; i++)
            {
                ...
                {                    
					...
					particleCount = Mathf.Clamp(particleCount,0,emitterSystems[n][i].maxParticles - oldParticlesAlive);
